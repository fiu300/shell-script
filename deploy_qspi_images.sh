...
export XIL_CES_ZYNQ_UBOOT_QSPI_FREQ_HZ=10000000
...
FSBL_ELF=${ZN_TARGET_DIR}/fsbl.elf


echo "#################"
echo ${ZN_QSPI_IMG_DIR}
echo "#################"
...
if [ -f "${ZN_SCRIPTS_DIR}/export_xilinx_env.sh" ]; then
    ...
    error_exit "Could not find file ${ZN_SCRIPTS_DIR}/export_xilinx_env.sh !!!"SDK
fi
# 烧录QSPI镜像
program_flash -f ${QSPI_IMAGE} -offset 0 -fsbl ${FSBL_ELF} -flash_type qspi_single -cable type xilinx_tcf url TCP:127.0.0.1:3121
