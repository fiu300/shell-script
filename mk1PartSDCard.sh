...
usage() {
    echo "Purpose: Make 1 Partition SD Card"
...
###############################################################################
# Purpose: Make 1 Partition SD Card
#
...
# To partition the drive, we'll use the parted utility.
type parted >/dev/null 2>&1 || error_exit "parted command not found"
...
case $choice in
    y|Y)
        ...
        sudo mkfs.vfat -n "SDCARD" /dev/${DISK}${PART}1 &> /dev/null
