...
case $choice in
	...
	sleep 5s 
        ...
        sudo mkfs.vfat -F 32 -n BOOT /dev/${DISK}${PART}1 &> /dev/null 
        ...
        sudo cp ${ZN_SDCARD_IMG_DIR}/BOOT.bin       ${BOOT_MOUNT_POINT}
        # Copy Kernel Device Tree Binaries
        sudo cp ${ZN_SDCARD_IMG_DIR}/devicetree.dtb ${BOOT_MOUNT_POINT}
        # Copy Kernel Image
        sudo cp ${ZN_SDCARD_IMG_DIR}/uImage         ${BOOT_MOUNT_POINT}

        # Plain text file to set U-Boot environmental variables to boot from the SD card
	sudo touch ${BOOT_MOUNT_POINT}/uEnv.txt
        ...
        sudo echo ${ZN_DOWNLOAD_DIR}  ${ROOT_MOUNT_POINT}
        
        sudo echo "sudo tar --strip-components=3 -C ${ROOT_MOUNT_POINT} -xzpf \
